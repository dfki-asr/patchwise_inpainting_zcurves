...
namespace inpainting
{
	...
	libmmv::Vec2f Gradient2D::computeGradient(libmmv::Volume* volume, libmmv::ByteVolume* mask, libmmv::Vec3i coordinate, bool fromMask )
	{
		...
		libmmv::Vec2f gradient = libmmv::Vec2f(0.0f, 0.0f);
		...
		for (int y = coordinate.y - 1; y <= coordinate.y + 1; y++)
		{
			for (int x = coordinate.x - 1; x <= coordinate.x + 1; x++)
			{
				...
				size_t index = volume->calculateVoxelIndex(libmmv::Vec3ui(x, y, coordinate.z));
				if (fromMask)
				{
	...
	libmmv::Vec2f Gradient2D::computeSimpleGradient(libmmv::Volume* volume, libmmv::ByteVolume* mask, libmmv::Vec3i coordinate, bool fromMask)
	{
		if (coordinate.x < 0 || coordinate.y < 0 || coordinate.z < 0 || coordinate.x >= (int)volume->getProperties().getVolumeResolution().x
			|| coordinate.y >= (int)volume->getProperties().getVolumeResolution().y || coordinate.z >= (int)volume->getProperties().getVolumeResolution().z)
			throw std::out_of_range("coordinates out of bounds");
		...
		if (coordinate.y - 1 > 0)
		{
			index = volume->calculateVoxelIndex(libmmv::Vec3ui(coordinate.x, coordinate.y - 1, coordinate.z));
			if (fromMask)
			{
		...
		if (coordinate.x - 1 > 0)
		{
			index = volume->calculateVoxelIndex(libmmv::Vec3ui(coordinate.x - 1, coordinate.y, coordinate.z));
			if (fromMask)
			{
				value_minus_1 = isStatusTargetRegion(byteVolume->nativeVoxelValue(index));
			}
...
*/
	...
	libmmv::Vec2f Gradient2D::computeOrthogonalGradient(libmmv::Volume* volume, libmmv::ByteVolume* mask, libmmv::Vec3i coordinate, bool fromMask)
	{
		libmmv::Vec2f gradient = computeGradient(volume, mask, coordinate, fromMask);
		return libmmv::Vec2f(gradient.y, -1.0f * gradient.x);
	}
	...
	void Gradient2D::initSobelStencil()
	{
		sobelStencil2D = 
		{
			libmmv::Vec2f( 3.0f,  3.0f), // x == -1, y == -1
			libmmv::Vec2f( 0.0f, 10.0f), // x == 0
			libmmv::Vec2f(-3.0f,  3.0f), // x == 1
			libmmv::Vec2f(10.0f,  0.0f), // x == -1, y == 0
			libmmv::Vec2f( 0.0f,  0.0f), // x == 0
			libmmv::Vec2f(-10.0f,  0.0f), // x == 1
			libmmv::Vec2f( 3.0f, -3.0f), // x == -1, y == 1
			libmmv::Vec2f( 0.0f, -10.0f), // x == 0
			libmmv::Vec2f(-3.0f, -3.0f)  // x == 1
		};
