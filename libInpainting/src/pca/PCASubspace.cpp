...
{ 
	...
	PCASubspace<TDATA>::PCASubspace(libmmv::ByteVolume* data, libmmv::ByteVolume* dictionary, libmmv::Vec3ui patchSize, std::vector<unsigned int>& permutation, std::vector<unsigned int>& patchKeys, unsigned int subspaceDimensions )
	...
	template<class TDATA>
	PCASubspace<TDATA>::PCASubspace(std::istream& is, libmmv::ByteVolume* data )
		: data(data)
	{
		loadFromStream( is );
	...
	template<class TDATA>
	std::vector<TDATA> PCASubspace<TDATA>::getDataPointInPrincipalSpace(libmmv::Vec3ui patchCenter)
	{
	...
	template<class TDATA>
	SharedPtrHNT PCASubspace<TDATA>::loadVariableBlock(const std::vector<unsigned int>& patchKeys, size_t blockStart, size_t blockSize )
	{
	...
	template<class TDATA>
	void PCASubspace<TDATA>::transferDictionaryPointToUnscaledPrincipalSpace( unsigned int patchKey, std::vector<float>& buffer)
	{
