...
namespace inpainting
{
	...
	template<class NORM>
	class KNNQueryJob : public ettention::Job
	{
	public:
		KNNQueryJob<NORM>( ParallelKNNQuery<NORM>* parentQuery, const ZCurveEntry& targetPoint, HyperCube& activeRange, IndexInterval datasetInterval, float priority );
    ...
    {
    public:
		ParallelKNNQuery<NORM>( ettention::ThreadPool* threadPool, DataSet<ZCurveEntry>* dataset, int k, unsigned int minimumIntervalLengthToUseRecursion, unsigned int minimumIntervalLengthToUseParallelism, Log* log);
