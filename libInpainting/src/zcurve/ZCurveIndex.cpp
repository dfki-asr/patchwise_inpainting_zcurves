...
namespace ettention
{
    namespace inpainting
    {
		...
		ZCurveIndex::ZCurveIndex( std::istream& is, ThreadPool* threadPool, ByteVolume* data, ByteVolume* mask, ByteVolume* dictionary, int filterSize, int minRecursionSize, bool useParallelization, int minParallelizationSize, ProgressReporter* progress, Log* log, CostFunctionOptions* costFunctionOptions )
			: MaskedIndex( mask )
			, data(data)
			, dictionary(dictionary)
		...
		std::tuple<ettention::Vec3i, float> ZCurveIndex::query( Vec3i targetPatchCenter )
		{
		...
		KNNQuery* ZCurveIndex::getQuery(Vec3i targetPatchUpperLeft)
		{
			return knnQuery;
		}
		...
		void ZCurveIndex::initQuery( ThreadPool* threadPool, int minRecursionSize, bool useParallelization, int minParallelizationSize, ProgressReporter* progress, Log* log )
		{
