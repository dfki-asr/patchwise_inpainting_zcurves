...
namespace ettention
{
    namespace inpainting
    {
			...
			, progress(progress)
        {
			...
			Vec3ui resolution = maskVolume->getProperties().getVolumeResolution();
			resolutionInt = Vec3i(resolution.x, resolution.y, resolution.z);
			unsigned int numberOfPixels = (resolution.x * resolution.y * resolution.z);

			std::vector<Vec3i> dictPatchCoords;
			Vec3i currentCoord;
		...
		Vec3i PatchMatchPatchSelection::adjustTargetPatchPosition(Vec3i targetPatchCenter)
		{
			return ensureTargetPatchPositionInsideSafeRegion( targetPatchCenter );
		}

        Vec3i PatchMatchPatchSelection::selectCenterOfBestPatch( Vec3i targetPatchCenter)
        {
			...
			Vec3i bestCoordinate = nearestNeighborField[targetPatchIndex];
		...
		float PatchMatchPatchSelection::computeSSD(Vec3i targetPatchCenter, Vec3i sourcePatchCenter)
		{
			BoundingBox3i insideBox = BoundingBox3i(Vec3i(0, 0, 0), resolutionInt - Vec3i(1, 1, 1));
			float result = 0.0F;
			Vec3i patchRadius = patchSize / 2;
			size_t currentIndex = -1;
			unsigned char status = TARGET_REGION;
			Vec3i targetCoord, sourceCoord;
			...
			for (int x = -patchRadius.x; x <= patchRadius.x; x++)
				for (int y = -patchRadius.y; y <= patchRadius.y; y++)
					for(int z = -1; z <= 1; z++)
					{
						targetCoord = targetPatchCenter + Vec3i(x, y, z);
						...
						if (targetCoord.x >= 0 && targetCoord.y >= 0 && targetCoord.x < resolutionInt.x && targetCoord.y < resolutionInt.y)
						{
							...
							{
								sourceCoord = sourcePatchCenter + Vec3i(x, y, z);
		...
		float PatchMatchPatchSelection::computeSSDnoCheck(Vec3i targetPatchCenter, Vec3i sourcePatchCenter)
		{
			BoundingBox3i insideBox = BoundingBox3i(Vec3i(0, 0, 0), resolutionInt - Vec3i(1, 1, 1));
			float result = 0.0F;
			Vec3i patchRadius = patchSize / 2;
		...
		void PatchMatchPatchSelection::propagateNearestNeighborField(Vec3i targetPatchCenter)
		{
			Vec3i minCoord = Vec3i(patchSize.x / 2, patchSize.y / 2, 1);
			size_t targetPatchIndex = Flatten3D(targetPatchCenter, resolutionInt);
			size_t propagatedPatchIndex;
			size_t currentIndex = -1;
			unsigned char status = TARGET_REGION;
			Vec3i currentCoord, mappedCoordInNNF, propagatedCoord;
			...
			for (int wx = -1; wx <= 1; wx++)
				...
				{
					currentCoord = targetPatchCenter + Vec3i(wx, wy, 0);
					if (currentCoord.x >= minCoord.x && currentCoord.y >= minCoord.y && currentCoord.x < resolutionInt.x - minCoord.x && currentCoord.y < resolutionInt.y - minCoord.y)
					{
						...
						propagatedCoord = mappedCoordInNNF - Vec3i(wx, wy, 0);
		...
		float PatchMatchPatchSelection::performRandomSearch(Vec3i targetPatchCenter)
		{
			Vec3i boxSize = Vec3i(resolutionInt.x, resolutionInt.y, 1);
			Vec3i randomCoord;
			...
			do 
			{
				...
				randomCoord = Vec3i(targetPatchCenter.x + randomXCoord, targetPatchCenter.y + randomYCoord, targetPatchCenter.z);
				...
				boxSize = Vec3i(boxSize.x / 2, boxSize.y / 2, 1);
			} while (boxSize.x > 1 && boxSize.y > 1);

			return nearestNeighborFieldCosts[targetPatchIndex];
		}

		ettention::Vec3i PatchMatchPatchSelection::getRandomCoordinateInsideSource()
		{
		...
		void PatchMatchPatchSelection::initNearestNeighborField()
		{
			...
			Vec3i reliableBorderDistance = Vec3i(patchSize.x / 2, patchSize.y / 2, 0);
			...
			for (int x = reliableBorderDistance.x; x < resolutionInt.x - reliableBorderDistance.x; x++)
				...
				{
					Vec3ui currentCoord = Vec3ui(x, y, 1);
		...
		void PatchMatchPatchSelection::initUnreliablePixelsNNF()
		{
			...
			Vec3i assignedCoord = Vec3i(0,0,0);
		...
		{
			Vec3i reliableBorderDistance = Vec3i(patchSize.x / 2, patchSize.y / 2, patchSize.z / 2);
			unsigned char status;
		...
		bool PatchMatchPatchSelection::isSourceUnreliable(Vec3ui coord)
		{
			Vec3i coordInt = Vec3i(coord.x, coord.y, coord.z);
			Vec3i startCoord = componentWiseMax(Vec3i(0, 0, 0), coordInt - (patchSize / 2));
			Vec3i endCoord = componentWiseMin(coordInt + (patchSize / 2), resolutionInt - Vec3i(1, 1, 1));
			unsigned char status;

			for (int x = startCoord.x; x <= endCoord.x; x++)
				for (int y = startCoord.y; y <= endCoord.y; y++)
					{
						Vec3i currentCoord = Vec3i(x, y, 1);
						size_t currentIndex = Flatten3D(currentCoord, resolutionInt);
						status = maskStatus[currentIndex];
						if (status != SOURCE_REGION)
							return true;
		...
		void PatchMatchPatchSelection::insertUnreliableRegionPixels()
		{
			...
			Vec3i currentCoord, unreliableCoordinate, unreliableCoordinateTmp, tmpCoord;
			float newValue, oldValue;
			ByteVolume* costVolume = new ByteVolume(resolutionInt, 42.0F);
			...
			for (int i = 0; i < unreliableCoordinates.size(); i++)
			{
				...
				for (int z = -1; z <= 1; z++)
				{
					unreliableCoordinateTmp = unreliableCoordinate + Vec3i(0, 0, z);
					unreliableIndex = Flatten3D(unreliableCoordinateTmp, resolutionInt);
					tmpCoord = currentCoord + Vec3i(0, 0, z);
		...
		Vec3i PatchMatchPatchSelection::ensureTargetPatchPositionInsideSafeRegion(Vec3i targetPatchCenter)
		{
			Vec3i borderDistance = Vec3i(patchSize.x / 2, patchSize.y / 2, 0);
			Vec3i insideCoord = componentWiseMax(borderDistance, targetPatchCenter);
			insideCoord = componentWiseMin(insideCoord, resolutionInt - borderDistance - Vec3i(1, 1, 0));
			insideCoord.z = 1;
			return insideCoord;
		}
		...
		void PatchMatchPatchSelection::computeCostAfterIteration()
		{
			...
			Vec3ui currentCoord, assignedCoord;
			size_t currentIndex;
			float currentCost;
		...
		void PatchMatchPatchSelection::doubleCheckUsingBruteForce(Vec3i targetPatchCenter)
		{
			float bestBruteForceCost = std::numeric_limits<float>::max();
			float currentCost = std::numeric_limits<float>::max();
			Vec3i currentSourceCoord, bestBruteForceCoord;
		...
		void PatchMatchPatchSelection::saveReliableMaskStatusToVolume(std::string filename)
		{
			...
			ByteVolume reliableVolume(resolutionInt, 25.0F);
			for (int x = 0; x < resolutionInt.x; x++)
				for (int y = 0; y < resolutionInt.y; y++)
					for (int z = 0; z < resolutionInt.z; z++)
					{
						currentCoord = Vec3i(x, y, 1);
						currentIndex = Flatten3D(currentCoord, resolutionInt);
						status = reliableMaskStatus[currentIndex];
		...
		void PatchMatchPatchSelection::setNNFofNeighboringTargetPixels(Vec3i targetCoord)
		{
			Vec3i currentTargetCoord;
			size_t targetIndex = Flatten3D(targetCoord, resolutionInt);
			size_t sourceIndex;
			unsigned char status, unreliableStatus;

			Vec3i sourceCoord = nearestNeighborField[targetIndex];
			Vec3i currentSourceCoord;
			Vec3i borderDistance = Vec3i(patchSize.x / 2, patchSize.y / 2, 0);
			BoundingBox3i insideBox = BoundingBox3i(borderDistance, resolutionInt - borderDistance - Vec3i(1, 1, 0));
			...
			for (int x = -patchSize.x / 2; x <= patchSize.x / 2; x++)
				for (int y = -patchSize.y / 2; y <= patchSize.y / 2; y++)
				{
					if (x != 0 && y != 0)
					{
						currentTargetCoord = targetCoord + Vec3i(x, y, 0);
						...
						if (currentTargetCoord.x >= 0 && currentTargetCoord.y >= 0 && currentTargetCoord.x < resolutionInt.x && currentTargetCoord.y < resolutionInt.y)
						{
							...
							{
								currentSourceCoord = sourceCoord + Vec3i(x, y, 0);
