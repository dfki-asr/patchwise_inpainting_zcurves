...
namespace inpainting
{
	...
	template<class TDATA>
	float L2DistanceWithWeight<TDATA>::distance(const TDATA& pointA, const TDATA& pointB, const TDATA& status)
	{
		float distance = 0.0f;
	...
	template<class TDATA>
	float L2DistanceWithWeight<TDATA>::distance(const TDATA& pointA, const TDATA& pointB, float maxDistance)
	{
		float distance = 0.0f;
		const unsigned int size = (unsigned int)pointA.size();
		for (unsigned int i = 0; i < size; i++)
