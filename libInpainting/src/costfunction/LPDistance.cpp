...
namespace inpainting
{
	...
	template<class TDATA>
	float LPDistance<TDATA>::distance(const TDATA& pointA, const TDATA& pointB, const TDATA& status)
	{
		float distance = 0.0f;
	...
	template<class TDATA>
	float LPDistance<TDATA>::distance(const TDATA& pointA, const TDATA& pointB, float maxDistance)
	{
		float distance = 0.0f;
		float maxDistancePower = std::numeric_limits<float>::max();
		const unsigned int size = (unsigned int)pointA.size();
