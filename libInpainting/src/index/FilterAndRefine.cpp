...
{
	template<class NORM>
	FilterAndRefineImpl<NORM>::FilterAndRefineImpl(libmmv::ByteVolume* data, libmmv::ByteVolume* mask, libmmv::ByteVolume* dictionary )
		: data(data), mask(mask), dictionary(dictionary)
	{
	}

	template<class NORM>
	FilterAndRefineImpl<NORM>::FilterAndRefineImpl(libmmv::ByteVolume* data, libmmv::ByteVolume* mask, libmmv::ByteVolume* dictionary, libmmv::Vec3ui patchSize)
	...
	template<class NORM>
	FilterAndRefineImpl<NORM>::~FilterAndRefineImpl()
	{

	}

	template<class NORM>
	std::tuple<libmmv::Vec3i, float> FilterAndRefineImpl<NORM>::selectBestPatch(libmmv::Vec3i targetCoordinate, std::vector<int> knn)
	{
		const libmmv::Vec3ui dictionaryResolution = dictionary->getProperties().getVolumeResolution();
		const libmmv::Vec3ui dataResolution = dictionary->getProperties().getVolumeResolution();
		...
		libmmv::Vec3i bestPatch;

		targetPatch.setPatchId(targetCoordinate);
		statusPatch.setPatchId(targetCoordinate);
		for (size_t i = 0; i < knn.size(); i++)
		{
			libmmv::Vec3i neighbourCoordinate = Unflatten3D((unsigned int)knn[i], dictionaryResolution);
		...
		return std::tuple<libmmv::Vec3i, float>(bestPatch, bestVectorSpaceDistance);
	}
