...
namespace inpainting 
{
    ...
    void InpaintingFrontEnd::initializeProblem()
    {
        ...
        problem->data = dynamic_cast<libmmv::ByteVolume*>(libmmv::VolumeDeserializer::load(parameterStorage.sparseFileName.string(), libmmv::Voxel::DataType::UCHAR_8));

        problem->mask = dynamic_cast<libmmv::ByteVolume*>(libmmv::VolumeDeserializer::load(parameterStorage.maskFileName.string(), libmmv::Voxel::DataType::UCHAR_8));

		if ( parameterStorage.dictionaryFileName != parameterStorage.sparseFileName )
		{
			problem->dictionaryVolume = dynamic_cast<libmmv::ByteVolume*>(libmmv::VolumeDeserializer::load(parameterStorage.dictionaryFileName.string(), libmmv::Voxel::DataType::UCHAR_8));
		} 
		...
		if (parameterStorage.dictionaryMaskGiven)
		{
			...
			{
				problem->dictionaryMask = dynamic_cast<libmmv::ByteVolume*>(libmmv::VolumeDeserializer::load(parameterStorage.dictionaryMaskFileName.string(), libmmv::Voxel::DataType::UCHAR_8));
			}
        ...
        {
            problem->denseScan = dynamic_cast<libmmv::ByteVolume*>(libmmv::VolumeDeserializer::load(parameterStorage.denseFileName.string(), libmmv::Voxel::DataType::UCHAR_8));
